...
def main():
    """
    ####################################################
    #  Creating an S/MIME encryption certificate       #
    ####################################################
    """
    try:
        ...
        if subordinate_pca_arn is None:
            ...
            ################################################
            #   createa a csr for the S/MIME encryption cert #    
            ################################################
            endpoint_serial_number = random.randint(1, 100000)
            subject_name_csr = x509.Name([
                x509.NameAttribute(NameOID.COMMON_NAME, 'S/MIME Encryption Cert')
            ])
            ...
            current_directory_path = os.path.dirname(os.path.realpath(__file__)) + '/'
            cert_path = current_directory_path + 'email_encrypt_cert.pem'
            
            textfilecert = open(cert_path, 'wb')
            textfilecert.write(response['Certificate'].encode('utf_8'))
            textfilecert.close()
           
            print("Successfully created S/MIME encryption certificate called email_encrypt_cert.pem \n")
