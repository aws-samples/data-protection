...
This workshop demonstrates how ACM Private Certificate Authority (PCA) makes it easy for managing your PKI infrastructure and generating private certificates. You will get hands on experience on the following use cases :
...
* Exporting private certificates that can be used on IOT devices so that the IOT devices can authenticate with AWS IOT Core
* Learn how to monitor security events associated with your certificate authority
* Learn Security best practices for your PKI infrastructure
* Multiple quizzes with re:Inforced learning
...
## Cloud9 IDE environment setup

#### 13. For the next section you need a Cloud9 IDE environment setup for executing code. PLease follow the instructions below :
...
## Security Monitoring for ACM Private CA:
...
If you are building IOT applications where client devices need to validate their identity to central control servers you could be using AWS IOT core or have your own control that server that your organization has built. In this section we will go through a hands on exercise showing you how you can get client device certificates that you can then put on an IOT device simulator
